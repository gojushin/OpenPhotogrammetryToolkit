...
Welcome to the API documentation for OpenPhotogrammetryToolkit (OPT), a comprehensive Python-based software designed to streamline and enhance the process of capturing and evaluating source material for photogrammetry and Structure from Motion (SFM) tasks.

OPT's philosophy is centered around flexibility, extensibility, and a strong commitment to being free and open-source software (FOSS). This guide will help you navigate and utilize the extensive capabilities of OPT effectively.
...
- Apache 2.0 License: We are pleased to inform you that OPT operates under the Apache License 2.0, which not only ensures legal security and transparency for both users and contributors but also encourages broad acceptance and participation. Although we are firm advocates of open-source software, we acknowledge that the copyleft approach may present certain obstacles in some cases. Hence, we grant you the freedom to apply your preferred licensing terms to any plugins you create, including the option to compile them into bytecode (.pyc files).
...
Please find below a brief on the installation and initial setup process.
